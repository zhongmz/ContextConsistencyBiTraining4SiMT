...
from fairseq import search,bleu,data
import torch
...
class Minimum_Risk_Training_Loss(FairseqCriterion):
    ...
    def _generate_hypotheses(self, model, sample):
        ...
        if self._generator is None:
            ...
            self._generator = WaitkSequenceGenerator(
                [model],
                self.task.target_dictionary,
                beam_size=self.mrt_beam_size,
                search_strategy=search_strategy,
                waitk=model.decoder.waitk,
        ...
        if self._greedy_generator is None and self.mrt_greedy:
            self._greedy_generator = WaitkSequenceGenerator(
                [model],
                self.task.target_dictionary,
                beam_size=1,
                search_strategy=search.BeamSearch(self.task.target_dictionary),
        ...
        with torch.no_grad():
            ...
            if self.mrt_greedy:
                greedy_hypos = self._greedy_generator.generate(
                    models = [model], 
                    sample = sample, 
                    prefix_tokens=None,
                    bos_token=None,
