    ...
    def _generate(
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            lprobs, avg_attn_scores, tgt_info = self.model.forward_decoder(
                ...
                self.temperature,
                test_waitk_lagging=test_waitk_lagging,
                src_info=src_info.index_select(0, reorder_state//beam_size) if reorder_state is not None else src_info,
            )
        ...
        for sent in range(len(finalized)):
            finalized_rw[ids[sent].item()] = finalized[sent][0]['context']
            finalized_tgt_info[ids[sent].item()] = finalized[sent][0]['tgt_info']
        return (
            finalized,
            finalized_rw,
