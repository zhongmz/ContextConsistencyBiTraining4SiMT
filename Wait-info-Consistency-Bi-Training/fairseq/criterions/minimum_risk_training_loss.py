...
class Minimum_Risk_Training_Loss(FairseqCriterion):
    ...
    def __init__(self, 
        ...
        #info quantizer
        self.mrt_freeze = mrt_freeze == "true"
    ...
    def add_args(parser):
        ...
        parser.add_argument('--mrt-freeze', type=str, default='false',)
    ...
    def prepare_main(self,model,sample):
        with torch.no_grad():
            ...
            del hypos
            return sample
    def _freeze_parameters(self,info_proj):
        for param in info_proj.parameters():
            param.requires_grad = False
    def freeze_parameters(self,model):
        self._freeze_parameters(model.encoder.info_proj)
        self._freeze_parameters(model.encoder.info_proj2)
        self._freeze_parameters(model.decoder.info_proj)
        self._freeze_parameters(model.decoder.info_proj2)
    ...
    def forward(self, model, sample, update_num = None,reduce=True):
        ...
        if self.mrt_freeze:
            self.freeze_parameters(model=model)
